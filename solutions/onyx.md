...
| Activation evidence (proof retrieved context is actually used, not just retrievable) | Partial: RAG chat answers include inline citations back to source documents, and OnyxBot posts answers directly in Slack — showing retrieved context is loaded into the actual response, not just indexed. No independent audit of citation accuracy was found. |
