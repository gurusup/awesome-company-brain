...
| Activation evidence (proof retrieved context is actually used, not just retrievable) | Partial — "Ask"/Slite Agent "returns cited source documents" and surfaces gaps or likely-outdated answers, a real citation trail; write actions through MCP can be queued for human approval, which is an approval log but not a full usage-audit trail of what was retrieved and cited across all responses. |
