...
| Activation evidence (proof retrieved context is actually used, not just retrievable) | Unknown — verified facts are stored "with source, date, owner, and confidence level" per vendor marketing, which is a real data structure for lineage, but no public documentation confirms that an agent's final answer actually cites or logs which stored facts it used. As the publisher of this list, we hold our own product to the same bar as every other entry here. |
