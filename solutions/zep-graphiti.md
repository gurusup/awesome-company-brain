...
| Activation evidence (proof retrieved context is actually used, not just retrievable) | Partial — every fact traces back to its source episode, giving retrieved context explicit provenance/lineage built into the data model itself. This documents lineage at the data layer; no independently confirmed example of that lineage surfacing as a live citation in an agent's actual response was found. |
