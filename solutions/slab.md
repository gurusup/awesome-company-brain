...
| Activation evidence (proof retrieved context is actually used, not just retrievable) | Unknown — not documented beyond the retrieval/citation features described above; AI Ask's public materials don't document inline source citations either, so there is no evidence trail for what context an answer actually drew on. |
