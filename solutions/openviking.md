...
| Activation evidence (proof retrieved context is actually used, not just retrievable) | Documented: every retrieval query preserves an inspectable browsing trajectory (which directories/tiers were traversed) — direct, logged evidence of what context was actually loaded, not just retrievable. The strongest activation-evidence documentation among the self-hosted tools reviewed. |
