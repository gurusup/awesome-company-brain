...
| Activation evidence (proof retrieved context is actually used, not just retrievable) | Partial: the chat UI shows citations to source documents for RAG-grounded answers — direct evidence retrieved content is loaded into the response — but no independent audit of citation accuracy/completeness was found. |
